...
class KelolahRekomendasi extends StatefulWidget {
  const KelolahRekomendasi({super.key});

  @override
  State<KelolahRekomendasi> createState() => _KelolahRekomendasiPageState();
}

class _KelolahRekomendasiPageState extends State<KelolahRekomendasi> {
  ...
  Future<void> fetchDestinationRequests() async {
    try {
      ...
      final List<DestinasiRequest> loadedRequests = snapshot.docs.map((doc) {
        final data = doc.data();
            ...
            isLoading
                ...
                : Expanded(
                    child: Padding(
                      ...
                      child: destinasiRequests.isEmpty
                          ? Center(
                              child: Column(
                          ...
                          : ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                return _buildDestinationCard(destinasiRequests[index]);
                              },
                            ),
                    ),
                  ),
  ...
  Future<void> _confirmRejectDestination(BuildContext context, DestinasiRequest request) async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Konfirmasi'),
        content: const Text('Apakah Anda yakin ingin menolak dan menghapus rekomendasi destinasi ini?'),
        actions: [
          TextButton(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _buildDetailItem(
                                ...
                                title: "Ticket Price",
                                value: request.isFree ? "Free Entry" : "Rp ${request.hargaTiket}",
                              ),
                              _buildDetailItem(
                  ...
                  children: [
                    Expanded(
                      child: ElevatedButton.icon(
                        onPressed: request.status.toLowerCase() == 'already_processed' 
                        ? null 
                        : () async {
                          try {
                            ...
                            Navigator.pop(modalContext);
                            _showErrorSnackbar(scaffoldContext, 'Gagal menyetujui rekomendasi: $e');
                          }
                        },
                      ...
                      child: ElevatedButton.icon(
                        ...
                        : () async {
                          try {
                            ...
                            Navigator.pop(modalContext);
                            _showErrorSnackbar(scaffoldContext, 'Gagal menolak rekomendasi: $e');
                          }
                        },
  ...
  Future<void> _approveDestination(DestinasiRequest request) async {
    ...
    try {
      ...
      final requestRef = firestore.collection('destinasi_requests').doc(request.id);
      batch.update(requestRef, {'status': 'approved'});
      
      // 2. Add the destination to the 'destinasi' collection
      final destinationRef = firestore.collection('destinasi').doc();
      ...
      batch.set(notifRef, {
        ...
        'message': 'Rekomendasi destinasi "${request.namaDestinasi}" Anda telah disetujui.',
    ...
    try {
      ...
      if (request.userId.isNotEmpty) {
        ...
        batch.set(notifRef, {
          ...
          'message': 'Maaf, rekomendasi destinasi "${request.namaDestinasi}" Anda telah ditolak.',
          'timestamp': FieldValue.serverTimestamp(),
          'read': false,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Rekomendasi destinasi berhasil ditolak dan dihapus'),
