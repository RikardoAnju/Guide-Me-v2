...
class _RequestRolePageState extends State<RequestRolePage> {
  ...
  String? _requestStatus; // Added to track the current status
  ...
  bool _isValidGoogleMapsUrl(String url) {
    ...
    final RegExp mapsRegex = RegExp(
      r'^https://www\.google\.com/maps/',
      caseSensitive: false,
    );
    
    return mapsRegex.hasMatch(url);
  }
  ...
  void _checkExistingRequests() async {
    ...
    try {
      ...
      // Check for both 'pending' and 'processing' status
      final querySnapshot = await FirebaseFirestore.instance
          .collection('role_requests')
          .where('userId', isEqualTo: currentUser.uid)
          .where('status', whereIn: ['pending', 'processing', 'already_processed'])
          .get();
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        setState(() {
          ...
          _requestStatus = doc['status']; // Store the current status
        });
      }
    } catch (e) {
  ...
  void _submitRequest() async {
    ...
    if (_hasPendingRequest) {
      String statusMessage = _requestStatus == 'pending' 
          ? 'Anda sudah memiliki permintaan role yang sedang ditinjau.'
          : 'Anda sudah memiliki permintaan role yang sedang diproses.';
      
      _showSnackBar(statusMessage, isError: true);
      return;
    }
  ...
  Widget _buildPendingRequestView() {
    ...
    if (_requestStatus == 'processing') {
      statusIcon = Icons.settings;
      statusColor = const Color(0xFFFF9800);
      statusTitle = 'Permintaan Sedang Diproses';
      statusDescription = 'Permintaan role Anda sedang dalam tahap pemrosesan. Tim kami sedang memverifikasi data yang Anda berikan.';
    } else {
                        ...
                        Expanded(
                          child: Text(
                            _requestStatus == 'processing' 
                                ? 'Proses verifikasi sedang berlangsung. Mohon bersabar menunggu.'
                  ...
                  _buildTextField(
                    ...
                    validator: (value) {
                      ...
                      if (!_isValidGoogleMapsUrl(value)) {
                        return 'URL Google Maps tidak valid';
                      }
                      return null;
                    },
                  ),
